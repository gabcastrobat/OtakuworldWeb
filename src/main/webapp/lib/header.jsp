...
<head>
    ...
    <style>
        ...
        /* Avatar de usuario */
        .ow-user-dropdown .dropdown-toggle::after { display:none; }
        .ow-avatar {
            width: 32px; height: 32px; border-radius: 50%;
            background: var(--ow-orange);
            display: flex; align-items: center; justify-content: center;
            font-family: var(--ow-font-display); font-size: 0.85rem; font-weight: 800;
            color: #0d1117; overflow: hidden;
            border: 2px solid rgba(227,114,57,0.55);
            transition: border-color 0.18s;
            cursor: pointer;
        }
        .ow-avatar img { width:100%; height:100%; object-fit:cover; }
        .ow-user-dropdown:hover .ow-avatar,
        .ow-user-dropdown.show .ow-avatar { border-color: var(--ow-orange); }
        .ow-user-dropdown .dropdown-menu {
            right:0; left:auto; min-width:200px;
            background-color:#1a1f2e;
            border:1px solid rgba(227,114,57,0.35);
            border-top:3px solid var(--ow-orange);
            border-radius:0 0 10px 10px;
            padding:0.4rem 0; margin-top:8px !important;
            box-shadow:0 8px 24px rgba(0,0,0,0.5);
        }
        .ow-user-dropdown .dropdown-header { color:rgba(200,200,208,.55); font-size:.78rem; padding:.4rem 1.1rem .25rem; }
        .ow-user-dropdown .dropdown-item { color:#c8c8d0; font-size:.88rem; padding:.5rem 1.1rem; transition:background .15s,color .15s; }
        .ow-user-dropdown .dropdown-item:hover { background:rgba(227,114,57,.12); color:#fff; }
        .ow-user-dropdown .dropdown-item.ow-logout { color:#f87171; }
        .ow-user-dropdown .dropdown-item.ow-logout:hover { background:rgba(220,38,38,.12); }
        .ow-user-dropdown .dropdown-divider { border-color:rgba(227,114,57,.18); margin:.25rem .8rem; }
    ...
    <div class="container-xl px-2">
        ...
        <div class="collapse navbar-collapse" id="owNav">
            <ul class="navbar-nav me-auto align-items-lg-stretch mb-2 mb-lg-0">
                ...
                <%
                    Modelo.Usuario usuarioNavCheck = (Modelo.Usuario) session.getAttribute("usuarioActivo");
                    if (usuarioNavCheck != null) {
                %>
                <li class="nav-item">
                    <a class="nav-link" href="MiLista" style="color:var(--ow-orange) !important;font-weight:700;">
                        📋 Mi Lista
                    </a>
                </li>
                <%
            ...
            <div class="nav-actions">
                <%
                    Modelo.Usuario usuarioNav = (Modelo.Usuario) session.getAttribute("usuarioActivo");
                    if (usuarioNav != null) {
                %>
                <!-- Usuario logueado: avatar tipo GitHub -->
                <div class="nav-item dropdown ow-user-dropdown">
                    <a href="#" class="nav-link dropdown-toggle p-0 d-flex align-items-center"
                       data-bs-toggle="dropdown" aria-expanded="false"
                       style="border-bottom:none;">
                        <div class="ow-avatar">
                            <% if (usuarioNav.getAvatarUrl() != null && !usuarioNav.getAvatarUrl().isEmpty()) { %>
                                <img src="<%= usuarioNav.getAvatarUrl() %>" alt="<%= usuarioNav.getNombre() %>">
                            <% } else { %>
                                <%= usuarioNav.getInicialCorreo() %>
                            <% } %>
                        </div>
                    </a>
                    <ul class="dropdown-menu dropdown-menu-end">
                        <li><h6 class="dropdown-header"><%= usuarioNav.getNombre() %></h6></li>
                        <li><span class="dropdown-item disabled" style="font-size:.78rem;opacity:.5;padding-top:0;">
                            <%= usuarioNav.getCorreo() %>
                        </span></li>
                        <li><hr class="dropdown-divider"></li>
                        <li><a class="dropdown-item" href="#">⚙ Configuracion</a></li>
                        <li><hr class="dropdown-divider"></li>
                        <li><a class="dropdown-item ow-logout" href="Login?logout">Cerrar sesion</a></li>
                    </ul>
                </div>
                <%
                    } else {
                %>
                <!-- Sin sesion: botones normales -->
                <a href="Login" class="btn-ow-login">Iniciar sesion</a>
                <a href="Registro" class="btn-ow-register">Registrarse</a>
                <%
                    }
                %>
            </div>
